...
export default class HistoryViewProvider implements IWebviewProvider {
  ...
  constructor(state?: any) {
    this.extensionPath = VSCode.extensions.getExtension(constans.EXTENSION_ID).extensionPath;
  }

  static show(gist: IGist) {
    (HistoryViewProvider as any).reveal(VSCode.ViewColumn.One, gist.id);
  }
  ...
  getHtmlContent(gistID: string): string {
    ...
    html = html.replace(/(<link.+?href="|<script.+?src="|<img.+?src=")(.+?)"/g, (m, tag, link) => {
      ...
      return tag + VSCode.Uri.file(path.resolve(dirPath, link)).with({ scheme: 'vscode-resource' }).toString() + '"';
    });
